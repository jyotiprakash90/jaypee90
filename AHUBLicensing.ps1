...
Function Update-VMSSLicense
{
	...
	try
	{
		if(!$VMSSName)
		{
			...
			{
				$vmss = Get-AzVmss -ResourceGroupName $AzureResourceGroup -VMScaleSetName $VMSSName
				$vmss.VirtualMachineProfile.LicenseType = $LicenseType
				Write-Output ""
				Write-Output " Updating VMSS : ['$($VMSSName)'] with License :['$($LicenseType)'] Started"
				Update-AzVmss -ResourceGroupName $AzureResourceGroup -VMScaleSetName $VMSSName -VirtualMachineScaleSet $vmss -Verbose:$false
...
Function Update-VMLicense
{
	...
	try
	{
		if(!$VMName)
		{
			...
			foreach ($VMName in (Get-AzVM -ResourceGroupName $AzureResourceGroup).Name)
			{
				$vm = Get-AzVM -ResourceGroupName $AzureResourceGroup -Name $VMName
				$vm.LicenseType = $LicenseType
				Write-Output ""
				Write-Output " Updating VM : ['$($VMName)'] with License : ['$($LicenseType)'] Started"
				Update-AzVM -ResourceGroupName $AzureResourceGroup -VM $vm -Verbose:$false
